...
void process()
{
    while (true)    // 这个线程是会一直循环下去
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            double currentTime = header.stamp.toSec();
            Eigen::Vector3d accelerometer_bias = estimator.Bas[WINDOW_SIZE];
            Eigen::Vector3d gyroscope_bias = estimator.Bgs[WINDOW_SIZE];
            std::ofstream outfile("/home/c50043104/vins_ws/src/VINS-Mono-noted-master/result/imu_bias_a0.0_0.0_MH01.txt", std::ios::app);
            if (outfile.is_open()) {
                //将偏置写入文件
