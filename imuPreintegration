...
public:
    ...
    */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        if (systemInitialized == false)
        {
            ...
            prevBias_ = gtsam::imuBias::ConstantBias(acc_bias, gyro_bias);
            std::cout << "first time" << prevBias_.gyroscope() << std::endl;
        ...
        std::ofstream outfile("/home/c50043104/lio-sam_ws/src/LIO-SAM-DetailedNote/result/imu_bias_a0.15_0.0_0020.txt", std::ios::app);
        if (outfile.is_open()) {
            //将偏置写入文件
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // 添加当前帧imu数据到队列
        //todo test 给零偏加固定偏移
        thisImu.linear_acceleration.x += 0.15;
        thisImu.linear_acceleration.x += 0.15;
        thisImu.linear_acceleration.x += 0.15;
        thisImu.angular_velocity.x += 0.0;
        thisImu.angular_velocity.y += 0.0;
        thisImu.angular_velocity.z += 0.0;
...
int main(int argc, char** argv)
{
    ...
    Eigen::Vector3d initial_gyroscope_bias(0.003, 0.003, 0.003);
